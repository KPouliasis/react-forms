...
Table.toId = function (filename) {
  return parseInt(filename.slice(0,-5));
};

Table.toFilename = function (id) {
  return `000${id}.json`.slice(-9);
};
...
Table.prototype.write = function (id, nextRow) {
  const prevRow = this.read(id);
  const filepath = this._filepath(id);
  if (nextRow) {
    const contents = JSON.stringify(nextRow);
...
Table.prototype.insert = function (row) {
  const nextId = 1 + this.getRowIds().reduce((a, b) => Math.max(a, b), -1);
  row.id = nextId;
